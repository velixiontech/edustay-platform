...
import { Building2, MapPin, FileBarChart2 } from "lucide-react";
...
const OwnerProperties = () => {
  ...
  // Simulated current owner — show only their properties
  const currentOwnerId = "o1";
  const myProperties = mockProperties.filter((p) => p.ownerId === currentOwnerId);
  ...
  return (
    <DashboardLayout role="owner">
      <div className="space-y-8">
        ...
        <div className="grid md:grid-cols-2 gap-6">
          {myProperties.map((property) => {
            ...
            const revenue = rooms.filter((r) => r.status === "occupied" && r.rentPaid).reduce((s, r) => s + r.monthlyRent, 0);
              ...
              <Card key={property.id} className="shadow-card hover:shadow-elevated transition-shadow">
                ...
                <CardContent className="space-y-4">
                  ...
                  <Button
                    className="w-full"
                    variant="outline"
                    onClick={() => { setSelected(property); setOpen(true); }}
                  >
                    <FileBarChart2 className="h-4 w-4 mr-2" /> View &amp; Export Monthly Report
                  </Button>
                </CardContent>
              </Card>
            );
      ...
      <PropertyReport property={selected} ownerName={selected?.ownerName} open={open} onOpenChange={setOpen} />
    </DashboardLayout>
  );
};
